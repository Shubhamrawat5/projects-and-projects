...
node::node(QGraphicsScene*scene,inputConnection iType,outputConnection oType,QString name,int width,int height)
...
inputConnection node::getIType(){
    return iType;
}

outputConnection node::getOType(){
    return oType;
}

connector* node::getCI1(){
    return ci1;
}

connector* node::getCI2(){
    return ci2;
}

connector* node::getCO(){
    return co;
}

unordered_multimap<node*,QGraphicsPathItem*>* node::getOutput(){
    return &output;
}
...
bool node::imageCalculate(QImage&image){
    return false;
}

float node::clampF(float i){
    if(i>1) return 1;
    else if(i<0) return 0;
    else return i;
}

int node::clamp(int i){
    if(i>255) return 255;
    else if(i<0) return 0;
    else return i;
}
